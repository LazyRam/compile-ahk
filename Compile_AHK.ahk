...
ShowCredits:
	...
	Gui 2:Add, Text, vURL_1 gOpenLink x48, AutoHotkey by Chris - AutoHotkey_L by Lexikos
...
_SaveIniInScript()
{
	...
	Script:=fileopen(In_File,"r")
	Encoding := Script.Encoding ;check encoding of scriptfile
	; If (In_File_Content = "")
	; {
...
_Change_Icons()
{
	...
	IsIcon8 := ResourceIdOfIcon(Bin_File , 7) ;check If there is a group 8 in .bin --> AHK_L
	If (Icon_1_Set and IsIcon8=230) ; just a copy of 1st icon command...replicated below 
...
Restore_File(File) {
	...
	{
		FileAppend , `n* Restore_File`nFileMove `, %File%_backup `, %File% , %A_WorkingDir%\%Log_File%
		FileMove , %File%_backup , %File% , 1
		If ErrorLevel
			_Error_Exit(Error_Message . "Couldn't copy """ . File . "_backup"" to """ . File """.")
		FileAppend , `n# Restored successfully`n , %A_WorkingDir%\%Log_File%
	}
	Return
}
; --------------------------------------------------------------------------------
